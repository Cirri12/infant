    ...
    if exist(cosmofn,'file')
        try
            ...
            for i = 1:size(res_rdm.samples,1)
                rdm(:,ds.sa.targets(res_rdm.sa.targets1(i)),ds.sa.targets(res_rdm.sa.targets2(i))) = res_rdm.samples(i,:);
                rdm(:,ds.sa.targets(res_rdm.sa.targets2(i)),ds.sa.targets(res_rdm.sa.targets1(i))) = res_rdm.samples(i,:);
            end
