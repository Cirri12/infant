...
for i=1:length(stream)
    ...
    if stream(i) == -3
        ...
        if i==1
            ...
            texta = annotation('textbox','Units','Pixels','Position',[a.Position(1) a.Position(2)+aw+45 aw*3 20],'LineStyle','none','String',...
...
aw = 300;
bufferw = 75;
a = axes('Units','pixels','Position',[left bottom aw aw],'FontSize',16,'LineWidth',2);hold on
histogram(T.age_months,.5:11.5,'FaceColor','w','EdgeColor','k','LineWidth',2)
xlabel('age in months')
ylabel('number of infants')
xlim([0 12])
ylim([0 10])    
%a.YTick = [];

annotation('textbox','Units','Pixels','Position',[texta.Position(1) a.Position(2)+aw+20 texta.Position(3)*2 20],'LineStyle','none','String',...
    'Demographics summary','FontSize',20,'VerticalAlignment','middle','FontWeight','bold');

a = axes('Units','pixels','Position',[left+aw+bufferw bottom aw aw],'FontSize',16,'LineWidth',2);hold on
histogram(T.blockscompleted,.5:9.5,'FaceColor','w','EdgeColor','k','LineWidth',2)
xlabel('blocks completed')
ylabel('number of infants')
xlim([0 10])
ylim([0 25])
%a.YTick = [];

a = axes('Units','pixels','Position',[left+2*(aw+bufferw) bottom aw aw],'FontSize',16,'LineWidth',2);hold on
x = T.age_months;
y = T.blockscompleted;
