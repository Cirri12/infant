...
for subjectnr = 1:51
    ...
    if exist(cosmofn,'file')
        ...
        age_monts(end+1) = subinfo{end}.age_months;
    end
end
...
for s=1:numel(subjectlist) % 42 plots
    ...
    nrows = 9;
    ncols = 6;
    while ismember(p,[1:6 7:12]) %skip to insert average later
        p=p+1;
    end
    row = ceil(p/ncols);
    col = mod(p-1,ncols);
...
X_cell = {X(age_monts<=6,:),X(age_monts>6,:)};
f_cell = {ft_cell(age_monts<=6),ft_cell(age_monts>6)};
tt = {'Average (<= 6 months)','Average (> 6 months)'};
for age=1:2
    XX = X_cell{age};
    ft = f_cell{age};
    a=axes('Position',[.025+.5*(age-1) .82  3*.9/ncols .165]);
    stem(ft{1}.freq,mean(XX(:,:)),'filled','Color',co(1,:),'LineWidth',2);hold on
    stem(ft{1}.freq(pidx),mean(XX(:,pidx)),'filled','Color',co(4,:),'LineWidth',2);
    a.YLim
    %ylim([0 0.12]);
    xlim([1 40]);
    xticks(5:5:40);
    ...
    x=[];
    for i=1:numel(ft)
        [~,ia] = ismember(ft{i}.label,layout.label);
        x(ia,:) = ft{i}.powspctrm;
    end
    idx = find(sum(abs(x')));
    % ft = ft_cell{1};
